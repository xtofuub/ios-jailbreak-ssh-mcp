...
- If connection, empty roots, or local setup looks wrong, run `ios_doctor()` first.
- Use `ios_config()` when the MCP package/config install might be wrong.
- Use `ios_app(bundleId)` for a first structured pass on a known app bundle id.
