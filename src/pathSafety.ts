...
export function assertAllowedRootConfig(roots: string[]): string[] {
  const normalizedRoots = roots.map(normalizeRemotePath);
  const deduped = [...new Set(normalizedRoots)];

  for (const root of deduped) {
    const underBuiltInRoot = DEFAULT_ALLOWED_ROOTS.some((builtInRoot) =>
      isPathWithin(root, builtInRoot)
    );

    if (!underBuiltInRoot) {
      throw new SafetyError(
        `Configured allowed root is outside the built-in safe roots: ${root}`
      );
    }
  }
...
export function assertSafePath(
  input: string,
  config: Pick<ServerConfig, "allowedRoots">
): string {
  const path = normalizeRemotePath(input);

  const blocked = BUILT_IN_BLOCKED_PATHS.find((blockedPath) =>
    isPathWithin(path, blockedPath)
  );
  ...
  if (!allowed) {
    throw new SafetyError(
      `Path is outside allowed roots: ${path}. Allowed roots: ${config.allowedRoots.join(", ")}`
    );
  }
...
export function backupPathFor(path: string, now = new Date()): string {
  const stamp = now
    .toISOString()
    .replace(/[-:]/g, "")
    .replace(".", "")
