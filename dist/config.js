...
export function getConfigPathFromArgs(argv = process.argv.slice(2)) {
...
async function discoverDefaultConfigPath() {
...
async function loadJsonConfig(path) {
    ...
    if (!result.success) {
        throw new ConfigError(`Invalid config file ${absolutePath}: ${result.error.issues
...
function readHermesDecoderPresetEnv() {
    ...
    if (!result.success) {
        throw new ConfigError("IOS_FILES_MCP_HERMES_DECODER_PRESET must be one of: auto, hermesc, hbc-decompiler, hbc-disassembler, hbctool, jsc2llvm, custom.");
    }
    return result.data;
}
function readR2ModeEnv() {
...
function resolveFromBase(path, baseDir) {
...
export async function loadConfig() {
    ...
    const allowedRoots = assertAllowedRootConfig(merged.allowedRoots ?? [...DEFAULT_ALLOWED_ROOTS]);
    const defaultLocalRoots = [
        process.cwd(),
        resolve(homedir(), "Desktop"),
        resolve(homedir(), "Downloads")
    ];
    const localArtifactRoots = (merged.localArtifactRoots ?? defaultLocalRoots).map((root) => resolveFromBase(root, loadedConfig.baseDir));
    ...
    if (resolvedPrivateKeyPath) {
        try {
            ...
            throw new ConfigError(`SSH private key not readable: ${resolvedPrivateKeyPath}. Check IOS_FILES_MCP_KEY_PATH or the privateKeyPath in your JSON config.`);
        }
    }
    return {
        host,
